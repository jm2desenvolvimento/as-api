// Lista completa de permissões do sistema AgendaSaúde
// Organizada por categorias para facilitar a manutenção

export const PERMISSIONS = {
  // === USUÁRIOS ===
  USER_CREATE: 'user_create',
  USER_VIEW: 'user_view',
  USER_UPDATE: 'user_update',
  USER_DELETE: 'user_delete',
  USER_LIST: 'user_list',

  // === PERFIS ===
  PROFILE_CREATE: 'profile_create',
  PROFILE_VIEW: 'profile_view',
  PROFILE_UPDATE: 'profile_update',
  PROFILE_DELETE: 'profile_delete',
  PROFILE_LIST: 'profile_list',

  // === PERMISSÕES E ROLES ===
  PERMISSION_CREATE: 'permission_create',
  PERMISSION_VIEW: 'permission_view',
  PERMISSION_UPDATE: 'permission_update',
  PERMISSION_DELETE: 'permission_delete',
  PERMISSION_LIST: 'permission_list',
  ROLE_CREATE: 'role_create',
  ROLE_VIEW: 'role_view',
  ROLE_UPDATE: 'role_update',
  ROLE_DELETE: 'role_delete',
  ROLE_LIST: 'role_list',
  USER_PERMISSION_MANAGE: 'user_permission_manage',

  // === DASHBOARD ===
  DASHBOARD_VIEW: 'dashboard_view',
  DASHBOARD_STATS: 'dashboard_stats',
  DASHBOARD_REPORTS: 'dashboard_reports',
  DASHBOARD_MASTER_VIEW: 'dashboard_master_view',
  DASHBOARD_ADMIN_VIEW: 'dashboard_admin_view',
  DASHBOARD_DOCTOR_VIEW: 'dashboard_doctor_view',
  DASHBOARD_PATIENT_VIEW: 'dashboard_patient_view',

  // === AGENDAMENTOS ===
  APPOINTMENT_CREATE: 'appointment_create',
  APPOINTMENT_VIEW: 'appointment_view',
  APPOINTMENT_UPDATE: 'appointment_update',
  APPOINTMENT_DELETE: 'appointment_delete',
  APPOINTMENT_LIST: 'appointment_list',
  APPOINTMENT_CANCEL: 'appointment_cancel',
  APPOINTMENT_RESCHEDULE: 'appointment_reschedule',

  // === AGENDA DO MÉDICO ===
  DOCTOR_SCHEDULE_VIEW: 'doctor_schedule_view',
  DOCTOR_SCHEDULE_MANAGE: 'doctor_schedule_manage',

  // === PACIENTES ===
  PATIENT_CREATE: 'patient_create',
  PATIENT_VIEW: 'patient_view',
  PATIENT_UPDATE: 'patient_update',
  PATIENT_DELETE: 'patient_delete',
  PATIENT_LIST: 'patient_list',

  // === MÉDICOS ===
  DOCTOR_CREATE: 'doctor_create',
  DOCTOR_VIEW: 'doctor_view',
  DOCTOR_UPDATE: 'doctor_update',
  DOCTOR_DELETE: 'doctor_delete',
  DOCTOR_LIST: 'doctor_list',

  // === ESPECIALIDADES ===
  SPECIALTY_CREATE: 'specialty_create',
  SPECIALTY_VIEW: 'specialty_view',
  SPECIALTY_UPDATE: 'specialty_update',
  SPECIALTY_DELETE: 'specialty_delete',
  SPECIALTY_LIST: 'specialty_list',

  // === UNIDADES DE SAÚDE ===
  HEALTH_UNIT_CREATE: 'health_unit_create',
  HEALTH_UNIT_VIEW: 'health_unit_view',
  HEALTH_UNIT_UPDATE: 'health_unit_update',
  HEALTH_UNIT_DELETE: 'health_unit_delete',
  HEALTH_UNIT_LIST: 'health_unit_list',

  // === PREFEITURAS ===
  CITY_HALL_CREATE: 'city_hall_create',
  CITY_HALL_VIEW: 'city_hall_view',
  CITY_HALL_UPDATE: 'city_hall_update',
  CITY_HALL_DELETE: 'city_hall_delete',
  CITY_HALL_LIST: 'city_hall_list',

  // === PRONTUÁRIOS MÉDICOS ===
  MEDICAL_RECORD_CREATE: 'medical_record_create',
  MEDICAL_RECORD_VIEW: 'medical_record_view',
  MEDICAL_RECORD_UPDATE: 'medical_record_update',
  MEDICAL_RECORD_DELETE: 'medical_record_delete',
  MEDICAL_RECORD_LIST: 'medical_record_list',

  // === AGENDA MÉDICA ===
  MEDICAL_SCHEDULE_CREATE: 'medical_schedule_create',
  MEDICAL_SCHEDULE_VIEW: 'medical_schedule_view',
  MEDICAL_SCHEDULE_UPDATE: 'medical_schedule_update',
  MEDICAL_SCHEDULE_DELETE: 'medical_schedule_delete',
  MEDICAL_SCHEDULE_LIST: 'medical_schedule_list',

  // === RELATÓRIOS ===
  REPORT_GENERATE: 'report_generate',
  REPORT_VIEW: 'report_view',
  REPORT_EXPORT: 'report_export',

  // === CONFIGURAÇÕES ===
  CONFIG_VIEW: 'config_view',
  CONFIG_UPDATE: 'config_update',
  SYSTEM_SETTINGS: 'system_settings',

  // === AUDITORIA ===
  AUDIT_VIEW: 'audit_view',
  AUDIT_EXPORT: 'audit_export',

  // === NOTIFICAÇÕES ===
  NOTIFICATION_SEND: 'notification_send',
  NOTIFICATION_VIEW: 'notification_view',
  NOTIFICATION_MANAGE: 'notification_manage',
} as const;

// Array com todas as permissões para facilitar iteração
export const ALL_PERMISSIONS = Object.values(PERMISSIONS);

// Permissões agrupadas por categoria
export const PERMISSION_GROUPS = {
  users: [
    PERMISSIONS.USER_CREATE,
    PERMISSIONS.USER_VIEW,
    PERMISSIONS.USER_UPDATE,
    PERMISSIONS.USER_DELETE,
    PERMISSIONS.USER_LIST,
  ],
  profiles: [
    PERMISSIONS.PROFILE_CREATE,
    PERMISSIONS.PROFILE_VIEW,
    PERMISSIONS.PROFILE_UPDATE,
    PERMISSIONS.PROFILE_DELETE,
    PERMISSIONS.PROFILE_LIST,
  ],
  rbac: [
    PERMISSIONS.PERMISSION_CREATE,
    PERMISSIONS.PERMISSION_VIEW,
    PERMISSIONS.PERMISSION_UPDATE,
    PERMISSIONS.PERMISSION_DELETE,
    PERMISSIONS.PERMISSION_LIST,
    PERMISSIONS.ROLE_CREATE,
    PERMISSIONS.ROLE_VIEW,
    PERMISSIONS.ROLE_UPDATE,
    PERMISSIONS.ROLE_DELETE,
    PERMISSIONS.ROLE_LIST,
    PERMISSIONS.USER_PERMISSION_MANAGE,
  ],
  dashboard: [
    PERMISSIONS.DASHBOARD_VIEW,
    PERMISSIONS.DASHBOARD_STATS,
    PERMISSIONS.DASHBOARD_REPORTS,
  ],
  appointments: [
    PERMISSIONS.APPOINTMENT_CREATE,
    PERMISSIONS.APPOINTMENT_VIEW,
    PERMISSIONS.APPOINTMENT_UPDATE,
    PERMISSIONS.APPOINTMENT_DELETE,
    PERMISSIONS.APPOINTMENT_LIST,
    PERMISSIONS.APPOINTMENT_CANCEL,
    PERMISSIONS.APPOINTMENT_RESCHEDULE,
  ],
  doctorSchedule: [
    PERMISSIONS.DOCTOR_SCHEDULE_VIEW,
    PERMISSIONS.DOCTOR_SCHEDULE_MANAGE,
  ],
  patients: [
    PERMISSIONS.PATIENT_CREATE,
    PERMISSIONS.PATIENT_VIEW,
    PERMISSIONS.PATIENT_UPDATE,
    PERMISSIONS.PATIENT_DELETE,
    PERMISSIONS.PATIENT_LIST,
  ],
  doctors: [
    PERMISSIONS.DOCTOR_CREATE,
    PERMISSIONS.DOCTOR_VIEW,
    PERMISSIONS.DOCTOR_UPDATE,
    PERMISSIONS.DOCTOR_DELETE,
    PERMISSIONS.DOCTOR_LIST,
  ],
  specialties: [
    PERMISSIONS.SPECIALTY_CREATE,
    PERMISSIONS.SPECIALTY_VIEW,
    PERMISSIONS.SPECIALTY_UPDATE,
    PERMISSIONS.SPECIALTY_DELETE,
    PERMISSIONS.SPECIALTY_LIST,
  ],
  healthUnits: [
    PERMISSIONS.HEALTH_UNIT_CREATE,
    PERMISSIONS.HEALTH_UNIT_VIEW,
    PERMISSIONS.HEALTH_UNIT_UPDATE,
    PERMISSIONS.HEALTH_UNIT_DELETE,
    PERMISSIONS.HEALTH_UNIT_LIST,
  ],
  cityHalls: [
    PERMISSIONS.CITY_HALL_CREATE,
    PERMISSIONS.CITY_HALL_VIEW,
    PERMISSIONS.CITY_HALL_UPDATE,
    PERMISSIONS.CITY_HALL_DELETE,
    PERMISSIONS.CITY_HALL_LIST,
  ],
  medicalRecords: [
    PERMISSIONS.MEDICAL_RECORD_CREATE,
    PERMISSIONS.MEDICAL_RECORD_VIEW,
    PERMISSIONS.MEDICAL_RECORD_UPDATE,
    PERMISSIONS.MEDICAL_RECORD_DELETE,
    PERMISSIONS.MEDICAL_RECORD_LIST,
  ],
  medicalSchedules: [
    PERMISSIONS.MEDICAL_SCHEDULE_CREATE,
    PERMISSIONS.MEDICAL_SCHEDULE_VIEW,
    PERMISSIONS.MEDICAL_SCHEDULE_UPDATE,
    PERMISSIONS.MEDICAL_SCHEDULE_DELETE,
    PERMISSIONS.MEDICAL_SCHEDULE_LIST,
  ],
  reports: [
    PERMISSIONS.REPORT_GENERATE,
    PERMISSIONS.REPORT_VIEW,
    PERMISSIONS.REPORT_EXPORT,
  ],
  config: [
    PERMISSIONS.CONFIG_VIEW,
    PERMISSIONS.CONFIG_UPDATE,
    PERMISSIONS.SYSTEM_SETTINGS,
  ],
  audit: [
    PERMISSIONS.AUDIT_VIEW,
    PERMISSIONS.AUDIT_EXPORT,
  ],
  notifications: [
    PERMISSIONS.NOTIFICATION_SEND,
    PERMISSIONS.NOTIFICATION_VIEW,
    PERMISSIONS.NOTIFICATION_MANAGE,
  ],
};

// Permissões padrão por role
export const DEFAULT_ROLE_PERMISSIONS = {
  MASTER: ALL_PERMISSIONS, // Master tem todas as permissões
  ADMIN: [
    ...PERMISSION_GROUPS.users,
    ...PERMISSION_GROUPS.profiles,
    ...PERMISSION_GROUPS.rbac,
    ...PERMISSION_GROUPS.dashboard,
    ...PERMISSION_GROUPS.appointments,
    ...PERMISSION_GROUPS.doctorSchedule,
    ...PERMISSION_GROUPS.patients,
    ...PERMISSION_GROUPS.doctors,
    ...PERMISSION_GROUPS.specialties,
    ...PERMISSION_GROUPS.healthUnits,
    ...PERMISSION_GROUPS.cityHalls,
    ...PERMISSION_GROUPS.medicalRecords,
    ...PERMISSION_GROUPS.medicalSchedules,
    ...PERMISSION_GROUPS.reports,
    ...PERMISSION_GROUPS.config,
    ...PERMISSION_GROUPS.audit,
    ...PERMISSION_GROUPS.notifications,
  ],
  DOCTOR: [
    PERMISSIONS.DASHBOARD_VIEW,
    PERMISSIONS.DASHBOARD_STATS,
    PERMISSIONS.APPOINTMENT_VIEW,
    PERMISSIONS.APPOINTMENT_UPDATE,
    PERMISSIONS.APPOINTMENT_LIST,
    PERMISSIONS.PATIENT_VIEW,
    PERMISSIONS.PATIENT_LIST,
    PERMISSIONS.MEDICAL_RECORD_CREATE,
    PERMISSIONS.MEDICAL_RECORD_VIEW,
    PERMISSIONS.MEDICAL_RECORD_UPDATE,
    PERMISSIONS.MEDICAL_RECORD_LIST,
    PERMISSIONS.MEDICAL_SCHEDULE_VIEW,
    PERMISSIONS.MEDICAL_SCHEDULE_UPDATE,
    PERMISSIONS.MEDICAL_SCHEDULE_LIST,
    PERMISSIONS.DOCTOR_SCHEDULE_VIEW,
    PERMISSIONS.DOCTOR_SCHEDULE_MANAGE,
    PERMISSIONS.PROFILE_VIEW,
    PERMISSIONS.PROFILE_UPDATE,
  ],
  PATIENT: [
    PERMISSIONS.DASHBOARD_VIEW,
    PERMISSIONS.APPOINTMENT_CREATE,
    PERMISSIONS.APPOINTMENT_VIEW,
    PERMISSIONS.APPOINTMENT_CANCEL,
    PERMISSIONS.APPOINTMENT_RESCHEDULE,
    PERMISSIONS.DOCTOR_VIEW,
    PERMISSIONS.DOCTOR_LIST,
    PERMISSIONS.SPECIALTY_VIEW,
    PERMISSIONS.SPECIALTY_LIST,
    PERMISSIONS.HEALTH_UNIT_VIEW,
    PERMISSIONS.HEALTH_UNIT_LIST,
    PERMISSIONS.MEDICAL_RECORD_VIEW,
    PERMISSIONS.PROFILE_VIEW,
    PERMISSIONS.PROFILE_UPDATE,
  ],
};

// Enum de roles
export enum UserRole {
  MASTER = 'MASTER',
  ADMIN = 'ADMIN',
  DOCTOR = 'DOCTOR',
  PATIENT = 'PATIENT',
}

// Tipo para TypeScript
export type Permission = typeof PERMISSIONS[keyof typeof PERMISSIONS];
export type RoleType = keyof typeof DEFAULT_ROLE_PERMISSIONS; 